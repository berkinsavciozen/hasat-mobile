...
import { NOTIF_CHANNELS, notifEventsForRole, type NotifPrefKey } from "@/lib/hasat/notif-events";

function Toggle({ on, disabled, onPress }: { on: boolean; disabled?: boolean; onPress?: () => void }) {
  return (
    <Pressable
      disabled={disabled}
      onPress={onPress}
      ...
      style={{ backgroundColor: on ? "#C8833B" : "rgba(253,250,245,0.15)", opacity: disabled ? 0.5 : 1 }}
    >
      <View className="h-4 w-4 rounded-full bg-hwhite" style={{ marginLeft: on ? 16 : 0 }} />
    </Pressable>
  );
}
  ...
  return (
    <View className="flex-1 bg-dark" style={{ paddingTop: insets.top }}>
      <View className="flex-row items-center px-6 pb-3 pt-2">
        ...
        <Text className="font-serif text-xl font-bold text-hwhite">Bildirim Tercihleri</Text>
      </View>
      ...
      {isLoading || !prefs ? (
        ...
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 32 }}>
          {events.map((e) => (
            <View key={e.key} className="mb-3 overflow-hidden rounded-2xl border border-white/10 bg-white/5">
              <View className="border-b border-white/10 px-4 py-2.5">
                <Text className="text-sm font-medium text-hwhite">{e.label}</Text>
              </View>
              <View>
                {(() => {
                  ...
                  return channels.map((c, i) => {
                    ...
                    return (
                      <View
                        ...
                        style={i < channels.length - 1 ? { borderBottomWidth: 1, borderBottomColor: "rgba(253,250,245,0.05)" } : undefined}
                      >
                        <View className="flex-row items-center gap-2">
                          <Text className="text-sm text-hwhite/90">{c.label}</Text>
                          {comingSoon && (
                            <View className="rounded-full bg-white/10 px-1.5 py-0.5">
                              <Text className="text-[9px] text-hmuted">Yakında</Text>
                            </View>
                          )}
                        </View>
                        <Toggle
                          ...
                          onPress={comingSoon ? undefined : () => onToggle(col, !prefs[col])}
                        />
                      </View>
                    );
                  });
